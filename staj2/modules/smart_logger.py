...
class SmartLogger:
    ...
    def init_db(self):
        # SQLite veritabanında 'activity_logs' tablosunu oluşturur
        try:
    ...
    def synthesize_summary(self, event_type, target, details, ai_info=None):
        ...
        if event_type == "HUMAN_TYPO_TOLERATED":
            return f"Human password typo tolerated for target {target}. (No penalty/ban applied)."
    ...
    def log_event(self, level, module, event_type, target, details="", ai_info=None):
        ...
        try:
            ...
            cursor.execute("""INSERT INTO activity_logs 
                (timestamp, level, module, event_type, target, summary, raw_details, mitre_id, ai_verdict)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)""",
                (timestamp, level.upper(), module, event_type, str(target), summary, str(details), mitre_id, ai_verdict))
            conn.commit()
            conn.close()
        except Exception as e:
    ...
    def get_recent_readable_logs(self, limit=20):
        try:
            conn = sqlite3.connect(self.db_name, check_same_thread=False)
            cursor = conn.cursor()
            cursor.execute("SELECT timestamp, level, module, summary FROM activity_logs ORDER BY id DESC LIMIT ?", (limit,))
