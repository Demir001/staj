...
# ÇOK KATMANLI YAPAY ZEKA GÜVENLİK MOTORU (Ensemble + Zero-Day Autoencoder + Deep Classifier),
...
class LogMonitor:
    ...
    def start(self):
        # 106 Kurallı ve Çok Katmanlı Yapay Zekalı Log İzleme Servisini Başlatır
        print(f"[+] 106-Rule & Dual-AI Security Log Monitor Started: {time.ctime()}")
        ...
        try:
            ...
            print(f"[-] ERROR: Permission denied! {log_path} cannot be accessed.")
        except Exception as e:
            print(f"[-] Log Monitor Error: {e}")

    def parse_line(self, line):
        # Gelen log satırını hem 106 Regex Kuralı hem de Çok Katmanlı Yapay Zeka ile analiz eder
        ip = "LOCAL_SYSTEM"
        user = "UNKNOWN"
        matched_rule = None
        base_score = 0
        ...
        for event_type, (pattern, score) in self.patterns.items():
            ...
            if match:
                ...
                if event_type == "SSH_SUCCESS":
                    ...
                    # Başarılı girişte IP'nin eski başarısız risk skoru da temizlenir
                ...
                # D. İnsani Parola Hatası (Typo) vs Kasıtlı Brute-Force Ayrımı
                    ...
                    if not is_malicious:
                        # İnsani yazım hatası ise puan eklenmez / düşük tutulur
                        base_score = 0
                    else:
        ...
        ai_res = self.ai_engine.analyze(line)
        ...
        log_data = {"user": user, "ip": ip, "event": matched_rule or ai_res.get("verdict"), "timestamp": time.ctime(), "ai_analysis": ai_res}
    ...
    def threat_event(self, ip, user, score, event, ai_info=None, criticality="MEDIUM"):
        ...
        if self.callback and score > 0:
            ...
            msg = f"THREAT({event}) | Target: {ip} | Risk: {current_score:.1f} | MITRE: {mitre_id} | Level: {criticality}"
