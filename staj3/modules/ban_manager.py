...
class BanManager:
        ...
        try:
            if os.name != 'nt':
                if not is_internal: # Dış Ağ İçin UFW Güvenlik Duvarında En Başa (1. Kural) Blok Ekleme
                    os.system(f"sudo ufw insert 1 deny from {ip} to any comment 'Tiered-{criticality_level}-{duration_minutes}m' >/dev/null 2>&1")
                else: # İç Ağ İçin SSH İzolasyonunu 1. Sıraya Ekleme
                    os.system(f"sudo ufw insert 1 deny proto tcp from {ip} to any port 22 comment 'Internal-SSH-Isolate-{duration_minutes}m' >/dev/null 2>&1")
    ...
    def unban_ip(self, ip: str, reason: str = "Ban Duration Expired"):
        ...
        try:
            if os.name != 'nt':
                if not is_internal:
                    os.system(f"sudo ufw delete deny from {ip} >/dev/null 2>&1")
                else:
                    os.system(f"sudo ufw delete deny proto tcp from {ip} to any port 22 >/dev/null 2>&1")
            else:
